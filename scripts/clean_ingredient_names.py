...
class Cleaner(Cmd):
    ...
    def ingred_names_to_clean(self):
        ...
        try:
            ...
            return (
                self.df.loc[
                    ~ingred_names_total.isin(self.ingred_map.keys()), self.hdf_col
                ]
                .dropna()
                .str.replace(" yeast", "")
            )
    ...
    def do_status(self, arg):
        ...
        if self.category:
            ...
            if self.ingred_names_to_compare is not None:
                print(
                    f"    N left to map for current ingredient: {len(self.ingred_names_to_compare)}"
                )
            else:
    ...
    def help_remain(self, arg):
        print(
            "Print the remaining ingreds_names_similar or ingreds_to_map that contain the substring arg"
        )
    ...
    def do_impact(self, arg):
        ...
        num_records_affected = self.ingred_names_to_clean.value_counts()[
            self.cur_ingred_compare
        ]
        print(
            f"Records affected by mapping {self.cur_ingred_compare}: {num_records_affected}"
        )
    ...
    def do_progress(self, arg):
        ...
        print(
            f"Progress on targets of unique values: {100*cleaned_unique/dataset_unique}% ({cleaned_unique}/{dataset_unique})"
        )
        print(
            f"Progress on records in total : {100*cleaned_total/dataset_total}% ({cleaned_total}/{dataset_total})"
        )
    ...
    def do_remove_ingreds(self, arg):
        ...
        if not self.active:
            del_key_list = [
                k for k in self.ingred_map.keys() if self.ingred_map[k] == arg
            ]
    ...
    def do_exclude(self, arg):
        ...
        for s in exclude_strings:
            ...
            self.ingred_names_to_compare = [
                i for i in self.ingred_names_to_compare if s not in i
            ]
    ...
    def help_exclude(self):
        print(
            "In current list of ingredients to compare, exclude all entries "
    ...
    def set_prompt_compare(self):
        ...
        self.prompt = (
            ...
            + self.cur_ingred_compare
            + " ("
            + str(self.ingred_names_to_clean.value_counts()[self.cur_ingred_compare])
            + ")"
        )
